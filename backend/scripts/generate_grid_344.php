...
// Пути к файлам
$imagePath = __DIR__.'/../public/images/maze-344.png';
$configPath = __DIR__.'/../config/race_grid-344.json';
...
// Генерируем сетку с размером клетки 8 (344/8=43, 840/8=105)
$result = imageToGrid($imagePath, 8);
$grid = $result['grid'];
...
$previewPath = __DIR__.'/../public/images/maze344_preview.html';
$pixelPreviewPath = __DIR__.'/../public/images/maze344_pixel_preview.html';
...
function generatePreview($grid, $debug) {
    $html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Лабиринт 344x840: предпросмотр</title><style>
    ...
    $html .= '<div class="info"><h2>Предпросмотр лабиринта 344x840 (сгруппировано по клеткам)</h2></div>';
...
function generatePixelPreview($debug, $cols, $rows, $cellSize) {
    ...
    $html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Лабиринт 344x840: детализация по пикселям</title><style>
        body { font-family: Arial, sans-serif; margin: 20px; }
    ...
    $html .= '<div class="info"><h2>Детализация по пикселям (344x840)</h2></div>';
    $html .= '<div class="legend">
