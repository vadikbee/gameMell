...
// Пути к файлам
$imagePath = __DIR__.'/../public/images/maze.png';
...
$previewPath = __DIR__.'/../public/images/maze_preview.html';
$pixelPreviewPath = __DIR__.'/../public/images/maze_pixel_preview.html';
$previewContent = generatePreview($grid, $result['debug']);
$pixelPreviewContent = generatePixelPreview($result['debug'], $grid['cols'], $grid['rows']);
...
function generatePreview($grid, $debug) {
    $html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Лабиринт: предпросмотр</title><style>
    ...
    $html .= '<div class="info"><h2>Предпросмотр лабиринта (сгруппировано по клеткам)</h2></div>';
...
function generatePixelPreview($debug, $cols, $rows) {
    $html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Лабиринт: детализация по пикселям</title><style>
        body { font-family: Arial, sans-serif; margin: 20px; }
        .container { max-width: 100%; overflow: auto; }
        .pixel-grid { display: grid; grid-template-columns: repeat(' . ($cols * 13) . ', 1px); gap: 0; }
    ...
    $html .= '<div class="info"><h2>Детализация по пикселям</h2></div>';
    $html .= '<div class="legend">
    ...
    $pixelMap = array_fill(0, $rows * 13, array_fill(0, $cols * 13, ''));
    ...
    if (isset($debug['pixel_objects'])) {
        if (isset($debug['pixel_objects']['start'])) {
            foreach ($debug['pixel_objects']['start'] as $pixel) {
                if ($pixel['y'] < $rows * 13 && $pixel['x'] < $cols * 13) {
